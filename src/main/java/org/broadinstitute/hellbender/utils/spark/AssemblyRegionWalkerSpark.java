...
public abstract class AssemblyRegionWalkerSpark extends GATKSparkTool {
    ...
    public JavaRDD<Tuple3<AssemblyRegion, ReferenceContext, FeatureContext>> getAssemblyRegions(JavaSparkContext ctx) {
        ...
        setIntervals(paddedIntervalsForReads);
        final List<ShardBoundary> intervalShards = intervals.stream()
                .flatMap(interval -> Shard.divideIntervalIntoShards(interval, readShardSize, readShardSize, readShardPadding, sequenceDictionary).stream())
